...
// Get all manual jobs with pagination and filters
...
export const getJobsByUserPreferences = async (req, res) => {
  try {
    ...
    const searchParams = {
      query: preferences.titles ? preferences.titles.join(' ') : 'software engineer',
    ...
    const [manualJobs, apiJobs] = await Promise.all([
      getManualJobsByPreferences(preferences),
      fetchJobsFromAPI(searchParams)
    ]);
    
    // Merge and sort jobs
    const allJobs = [...manualJobs, ...apiJobs].sort((a, b) => 
  ...
  try {
    ...
    const recommendedJobs = await Job.find({
      $or: [
        ...
        { 'company.industry': preferences.industry },
        { jobType: preferences.jobType }
      ]
...
// Search jobs from external API based on custom query
export const searchJobsFromAPI = async (req, res) => {
  try {
    ...
    const jobs = await fetchJobsFromAPI({ query, location, page });
    ...
    res.status(500).json({
      success: false,
      message: 'Error searching jobs from API',
      error: error.message
    });
  }
};
...
const getManualJobsByPreferences = async (preferences) => {
  ...
  return await Job.find(filters).sort({ createdAt: -1 }).limit(50);
};